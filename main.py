...
# First, make sure you have your bot defined
bot = commands.Bot(command_prefix='!', intents=discord.Intents.all())

# Define an emotion_manager and storage_manager at the module level
...
async def set_stat(ctx, stat_name: str, value: float):
    ...
    # Optional: Show the updated relationship stage
...
"Your presence is... acceptable.",
        ...
        @self.bot.command(name="debug_info")
        @commands.has_permissions(administrator=True)
        async def debug_info(ctx):
            ...
            info.append(f"Bot uptime: {datetime.now(timezone.utc) - self.bot.start_time}")
            info.append(f"Connected servers: {len(self.bot.guilds)}")
            info.append(f"Total users tracked: {len(self.emotion_manager.user_emotions)}")
            info.append(f"Total conversations: {len(self.conversation_manager.conversations)}")
            info.append(f"Total profiles: {len(self.conversation_manager.user_profiles)}")
        ...
        @self.bot.command(name="list_users")
        @commands.has_permissions(administrator=True)
        async def list_users(ctx, count: int = 10):
            """List users the bot has data for"""
            if not self.emotion_manager.user_emotions:
                await ctx.send("No user data found.")
                return
            
            users = []
            for i, (user_id, data) in enumerate(list(self.emotion_manager.user_emotions.items())[:count]):
                # Try to get username
                user = self.bot.get_user(user_id)
                username = user.name if user else f"Unknown User ({user_id})"
                
                # Get relationship score
                score = self.emotion_manager.get_relationship_score(user_id)
            ...
            embed = discord.Embed(
                ...
                description=f"Showing {len(users)} of {len(self.emotion_manager.user_emotions)} users",
                color=discord.Color.blue()
            )
        ...
        @self.bot.command(name="inspect_user")
        @commands.has_permissions(administrator=True)
        async def inspect_user(ctx, user_id: discord.Member = None):
            ...
            # Get all the data
            emotional_data = self.emotion_manager.user_emotions.get(uid, {})
            memories = self.emotion_manager.user_memories.get(uid, [])
            events = self.emotion_manager.user_events.get(uid, [])
            milestones = self.emotion_manager.user_milestones.get(uid, [])
            interactions = self.emotion_manager.interaction_stats.get(uid, Counter())
            profile = self.conversation_manager.user_profiles.get(uid, None)
            ...
            counts = [
                ...
                f"Conversations: {len(self.conversation_manager.conversations.get(uid, []))}"
            ]
            ...
            # Add file stats
            file_paths = [
                (self.storage_manager.profiles_dir / f"{uid}.json", "Emotions file"),
                (self.storage_manager.profiles_dir / f"{uid}_memories.json", "Memories file"),
                (self.storage_manager.profiles_dir / f"{uid}_events.json", "Events file"),
                (self.storage_manager.profiles_dir / f"{uid}_milestones.json", "Milestones file"),
                (self.storage_manager.user_profiles_dir / f"{uid}_profile.json", "Profile file"),
                (self.storage_manager.conversations_dir / f"{uid}_conversations.json", "Conversations file")
            ]
        ...
        @self.bot.command(name="simulate_event")
        @commands.has_permissions(administrator=True)
        async def simulate_event(ctx, event_type: str = None, user_id: discord.Member = None):
            ...
            self.emotion_manager.user_events.setdefault(uid, []).append(event_record)
            
            # Create a memory of this event
            await self.emotion_manager.create_memory_event(
                uid, event["name"], 
                f"A2 experienced a {event['name'].replace('_', ' ')}. {event['message']}",
                event["effects"], self.storage_manager
            )
        ...
        @self.bot.command(name="set_emotion")
        @commands.has_permissions(administrator=True)
        ...
        async def storage_stats(ctx):
            ...
            user_count = len(self.emotion_manager.user_emotions)
            profile_count = len(self.conversation_manager.user_profiles)
            
            # Count data files
            memory_files = len(list(self.storage_manager.profiles_dir.glob("*_memories.json")))
            event_files = len(list(self.storage_manager.profiles_dir.glob("*_events.json")))
            milestone_files = len(list(self.storage_manager.profiles_dir.glob("*_milestones.json")))
            conversation_files = len(list(self.storage_manager.conversations_dir.glob("*_conversations.json")))
            ...
            dir_stats = []
            for name, path in [
                ("Data", self.storage_manager.data_dir),
                ("Profiles", self.storage_manager.profiles_dir),
                ("User Profiles", self.storage_manager.user_profiles_dir),
                ("Conversations", self.storage_manager.conversations_dir)
            ]:
                if path.exists():
            ...
            for uid in self.emotion_manager.user_emotions:
                stage = self.emotion_manager.get_relationship_stage(uid)["current"]["name"]
                stages[stage] += 1
        ...
        async def backup_data(ctx):
            ...
            try:
                ...
                import shutil
                for src_dir in [self.storage_manager.profiles_dir, 
                             self.storage_manager.user_profiles_dir, 
                             self.storage_manager.conversations_dir]:
                    if src_dir.exists():
                ...
                if self.storage_manager.dm_settings_file.exists():
                    shutil.copy2(self.storage_manager.dm_settings_file, 
                               backup_dir / self.storage_manager.dm_settings_file.name)
        ...
        @self.bot.command(name="test_event")
        @commands.has_permissions(administrator=True)
